...
Deploys [ExternalDNS](https://kubernetes-sigs.github.io/external-dns/) scoped to the public hosted zone, watching `Service`, `Ingress`, and `HTTPRoute` resources tagged `external-dns.alpha.kubernetes.io/scope=public`.
...
- **[`external_dns`](https://github.com/ConsciousML/terragrunt-template-catalog-eks/tree/main/units/eks/addons/external_dns)** (catalog): provisions the IAM role this instance's `external-dns-public` service account assumes via Pod Identity, scoped to the public hosted zone
- **[`helm-httproute`](../helm-httproute)**: this instance only picks up `HTTPRoute`s annotated `scope: public`
