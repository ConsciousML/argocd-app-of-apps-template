...
- **[values.yaml](values.yaml)**: `clusterName`, `region`, and `vpcId` are placeholders. The catalog's [`app_of_apps` unit](https://github.com/ConsciousML/terragrunt-template-catalog-eks/blob/main/units/eks/addons/argocd/app_of_apps/terragrunt.hcl) injects the real values via `appParams.helm-aws-lbc` at sync time
