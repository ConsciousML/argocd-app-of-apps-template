...
- **[grafana-secrets-values.yaml](grafana-secrets-values.yaml)**: also sets `kube-prometheus-stack.grafana.admin.existingSecret` and `passwordKey`, consumed by `kube-prometheus-stack` through the same `extraValueFiles` entry so both charts agree on the target secret name
