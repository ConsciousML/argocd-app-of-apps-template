...
Both `gateway-public` and `gateway-private` instances of [`helm-gateway-api-gateway`](../helm-gateway-api-gateway) load this chart's `values.yaml` via `extraValueFiles` to reference the same `gatewayClassName`.
